...
Select	
    ...
    Amount,
    DueDate,
	rank() over (partition by billmonth order by amount DESC) as totalbill
from
	bill;
...
Select	
    ...
    Amount,
    DueDate,
    row_number() over (partition by CustomerID order by BillID) AS bills
    FROM 
		Bill;
 ...
 Select	
    ...
    Amount,
    DueDate,
	sum(UnitsConsumed) over (partition by CustomerID order by amount) as cumulative_units_consumed
    from 
		bill;
...
Select	
    ...
    Amount,
    DueDate,
    avg(amount) over (partition by unitsconsumed order by amount) as average_bills_amount
    from
		bill;
